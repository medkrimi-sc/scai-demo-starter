...
import { IA_FAQ_FALLBACK } from '@/lib/ai-brand-fallbacks';
...
export async function fetchFaqFromEdge(): Promise<FaqEdgeResult> {
  ...
  const emptyFallback = (): FaqEdgeResult => ({
    items: [...IA_FAQ_FALLBACK],
  ...
  try {
    ...
    return items.length > 0 ? { items, lastModified } : emptyFallback();
  } catch (error) {
