...
import { HeroOfferingsGrid } from './HeroOfferingsGrid.dev';
...
export const OfferingsGrid: React.FC<HeroProps> = (props) => {
  const { isEditing } = props.page.mode;
  const t = useTranslations();
  const dictionary = {
    SubmitCTALabel: t(dictionaryKeys.HERO_SubmitCTALabel) || '',
  ...
  return <HeroOfferingsGrid {...props} isPageEditing={isEditing} />;
};
