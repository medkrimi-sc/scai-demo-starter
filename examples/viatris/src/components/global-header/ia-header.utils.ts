...
/** Default icon order for iA primary nav (matches viatris.com layout). */
...
/** True when the URL points at a Viatris-branded media item (safe to render from CMS). */
export function isIaBrandedLogoSrc(logoSrc?: string): boolean {
  ...
  return (
    normalized.includes('viatris') ||
    normalized.includes('viatris-logo') ||
    normalized.includes('/media/project/common/viatris')
  );
}

/** Use inline Viatris logo in preview/live when CMS logo is empty or a non-Viatris placeholder asset. */
...
export function isUtilityLinkSelected(href: string | undefined, pathname: string): boolean {
  ...
  try {
    const url = new URL(href, 'https://viatris.com');
