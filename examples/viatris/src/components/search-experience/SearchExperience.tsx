        ...
        >
          <div className="mb-8">
            ...
            <p className="text-gray-600 mb-6">
              {t(DICTIONARY_KEYS.RESULTS_FOUND, { count: total ?? 0 }) ||
...
export const Default = (props: SearchExperienceProps) => (
  <Suspense fallback={<SearchExperienceFallback props={props} />}>
    <SearchExperienceDefault {...props} />
  </Suspense>
);

const SearchExperienceFallback = ({ props }: { props: SearchExperienceProps }) => {
  const { styles, id } = useParams(props.params);
  return (
    <div className={`component search-indexing ${styles}`} id={id ? id : undefined}>
      <div className="component-content max-w-7xl mx-auto p-6 pt-24 lg:pt-32">
        <div className="mb-8 h-10 animate-pulse rounded bg-muted" />
        <div className="grid gap-6 md:grid-cols-3">
          {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className="h-48 animate-pulse rounded bg-muted" />
          ))}
        </div>
      </div>
    </div>
