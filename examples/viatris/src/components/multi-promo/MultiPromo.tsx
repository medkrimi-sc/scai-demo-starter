...
/** Same content model as Default, but renders children in a responsive grid (Advice Zone / 3-up). */
export const StaticGrid: React.FC<MultiPromoProps> = (props) => {
  const { fields, params } = props;
  const { numColumns } = params || {};
  const { children } = fields?.data?.datasource || {};
  const { title, description } = fields?.data?.datasource || {};
  ...
  if (fields) {
    return (
      <section
        ...
        aria-labelledby={title?.jsonValue?.value ? 'multi-promo-heading-static' : undefined}
      >
        <div className="flex flex-col gap-4 group-[.is-inset]:px-4 sm:group-[.is-inset]:px-0 xl:flex-row xl:items-end xl:justify-between xl:gap-20">
          {title && (
            <div className="flex-grow md:basis-[60] lg:basis-[50]">
        ...
        {children?.results && children.results.length > 0 && (
          <div
            className={cn(
              'my-10 grid gap-6 sm:my-12 sm:gap-8',
              colClass,
              'group-[.is-inset]:px-0',
            )}
          >
            {children.results.map((item: MultiPromoItemProps, index: number) => (
              <div key={index} className="min-w-0">
                <MultiPromoItem {...item} page={props.page} />
              </div>
            ))}
          </div>
        )}
