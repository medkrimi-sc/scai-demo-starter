...
test('GET /sitemap-llm.xml returns 200 and valid XML', async () => {
    ...
    if (res.status !== 200) {
      // Integration test: requires `npm run dev` or `npm run start` on GEO_BASE_URL (default localhost:3000).
      console.warn(`[geo] skip sitemap-llm assertions: ${res.status} from ${BASE_URL}`);
      return;
    }
    expectValidXml(res);
    ...
    const urls = (Array.isArray(urlset.url) ? urlset.url : [urlset.url]).filter(Boolean);
    if (urls.length === 0) {
      console.warn('[geo] skip sitemap-llm URL assertions: empty urlset (Edge sitemap / env)');
      return;
    }
...
test('GET /sitemap.xml returns 200 and valid XML', async () => {
    ...
    if (res.status !== 200) {
      console.warn(`[geo] skip sitemap.xml assertions: ${res.status} from ${BASE_URL}`);
      return;
    }
    expectValidXml(res);
    ...
    const urls = (Array.isArray(urlset.url) ? urlset.url : [urlset.url]).filter(Boolean);
    if (urls.length === 0) {
      console.warn('[geo] skip sitemap.xml URL assertions: empty urlset (Edge sitemap / env)');
      return;
    }
    expect(urls.length).toBeGreaterThan(0);
