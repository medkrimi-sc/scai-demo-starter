...
import { IA_SERVICES_FALLBACK } from '@/lib/ai-brand-fallbacks';
...
export async function fetchServicesFromEdge(): Promise<ServiceEdgeResult> {
  ...
  const emptyFallback = (): ServiceEdgeResult => ({
    services: [...IA_SERVICES_FALLBACK],
  ...
  try {
    ...
    return services.length > 0 ? { services, lastModified } : emptyFallback();
  } catch (error) {
