...
import { Default as MultiPromo, StaticGrid as MultiPromoStaticGrid } from '@/components/multi-promo/MultiPromo';
...
describe('MultiPromo StaticGrid', () => {
  it('renders title and promo items in a grid without carousel', () => {
    const { getByText, queryByTestId, getAllByTestId } = render(
      <MultiPromoStaticGrid {...mockMultiPromoProps} />,
    );

    expect(getByText('Explore Our Collection')).toBeInTheDocument();
    expect(queryByTestId('carousel')).not.toBeInTheDocument();
    expect(getAllByTestId('multi-promo-item')).toHaveLength(4);
  });
});
